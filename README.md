...
| 功能 | 说明 |
|:-----|:-----|
| 🌐 **多数据源聚合** | 实习僧（170+ 条/次）、国家大学生就业服务平台（100 条/页）、198 家大厂官网（⚠️ 反爬严重，见下方说明）、API 接口直连（12 家大厂） |
| 🔌 **API 直连** | 阿里巴巴、腾讯、字节跳动、美团、京东、小红书、科大讯飞、滴滴、网易、哔哩哔哩、快手、大疆 |
...
**如果你需要大厂岗位信息，建议优先使用「API 接口」数据源**，已对接 12 家大厂的 JSON API，数据质量更稳定。
...
│   └── api_crawler.py          # API 接口爬虫（12 家大厂 JSON API 直连）
│
