...
| 功能 | 说明 |
|:-----|:-----|
| 🌐 **多数据源聚合** | 实习僧（170+ 条/次）、国家大学生就业服务平台（100 条/页）、198 家大厂官网 |
...
| 爬虫引擎 | Playwright + playwright-stealth（反检测） |
| 数据库 | SQLite（WAL 模式，高并发读写） |
| Excel 导出 | openpyxl |
...
│
├── config/
│   └── company_urls.json       # 大厂招聘页面 URL 配置
│
├── templates/
